module ApplicationHelper
  ...
  def avatar_for(url: nil, size: 32, initials: nil)
    css = "rounded-full shrink-0 object-cover shadow-md"
    style = "width: #{size}px; height: #{size}px;"

    if url
      image_tag(url, class: css, style:)
    else
